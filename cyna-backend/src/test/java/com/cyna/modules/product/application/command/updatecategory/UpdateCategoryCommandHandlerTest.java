...
class UpdateCategoryCommandHandlerTest {
    ...
    void should_update_category_successfully() {
        ...
        var existing = Category.reconstitute(id, "Old Name",
                Map.of("fr", new CategoryTranslation("Old Name", "Old desc")),
    ...
    void should_preserve_existing_image_on_update() {
        ...
        var existing = Category.reconstitute(id, "Name",
                Map.of("fr", new CategoryTranslation("Name", "desc")),
                existingImage, true, Instant.now(), Instant.now());
        when(categoryRepository.findById(id)).thenReturn(Optional.of(existing));
