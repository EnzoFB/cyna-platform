...
import { Product, ProductDetail, ProductSecurityCategory } from '../models/product.model';
...
const CATEGORY_CONTENT: Record<ProductSecurityCategory, CategoryContent> = {
  soc: {
    description:
...
export class CatalogService {
  ...
  getProductById(productId: string): Observable<ProductDetail | null> {
    return of(MOCK_PRODUCT_DETAILS.find(product => product.id === productId) ?? null);
  }

  getSimilarProducts(productId: string, category: ProductSecurityCategory, limit = 4): Observable<readonly Product[]> {
